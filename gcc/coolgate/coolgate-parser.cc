...
#include "coolgate/coolgate-parser.h"
#include "coolgate/coolgate-lexer.h"
#include "coolgate/coolgate-tree.h"
#include "coolgate/coolgate-symbol.h"
#include "coolgate/coolgate-symbol-mapping.h"
#include "coolgate/coolgate-scope.h"
...
Parser::skip_after_semicolon ()
{
  ...
  while (t->get_id () != Coolgate::END_OF_FILE && t->get_id () != Coolgate::SEMICOLON)
    {
      lexer.skip_token ();
      t = lexer.peek_token ();
    }

  if (t->get_id () == Coolgate::SEMICOLON)
    lexer.skip_token ();
}
...
Parser::skip_after_end ()
{
  ...
  while (t->get_id () != Coolgate::END_OF_FILE && t->get_id () != Coolgate::END)
    {
      lexer.skip_token ();
      t = lexer.peek_token ();
    }

  if (t->get_id () == Coolgate::END)
    lexer.skip_token ();
}

const_TokenPtr
Parser::expect_token (Coolgate::TokenId token_id)
{
  const_TokenPtr t = lexer.peek_token ();
...
bool
Parser::skip_token (Coolgate::TokenId token_id)
{
...
{
  const_TokenPtr t = lexer.peek_token ();
  return (t->get_id () == Coolgate::END_OF_FILE);
}
...
{
  const_TokenPtr t = lexer.peek_token ();
  return (t->get_id () == Coolgate::END || t->get_id () == Coolgate::END_OF_FILE);
}
...
{
  const_TokenPtr t = lexer.peek_token ();
  return (t->get_id () == Coolgate::END || t->get_id () == Coolgate::ELSE
	  || t->get_id () == Coolgate::END_OF_FILE);
}
...
Parser::parse_variable_declaration ()
{
  ...
  if (!skip_token (Coolgate::VAR))
    {
      skip_after_semicolon ();
      return Tree::error ();
    }

  const_TokenPtr identifier = expect_token (Coolgate::IDENTIFIER);
  if (identifier == NULL)
    {
      skip_after_semicolon ();
      return Tree::error ();
  ...
  SymbolPtr sym (new Symbol (Coolgate::VARIABLE, identifier->get_str ()));
...
Parser::parse_type_declaration ()
{
  ...
  SymbolPtr sym (new Symbol (Coolgate::TYPENAME, identifier->get_str ()));
  scope.get_current_mapping ().insert (sym);
...
Parser::parse_record ()
{
  ...
  const_TokenPtr record_tok = expect_token (Coolgate::RECORD);
  if (record_tok == NULL)
    {
      skip_after_semicolon ();
      return Tree::error ();
  ...
  while (next->get_id () != Coolgate::END)
    {
...
Parser::parse_type ()
{
  ...
  t = lexer.peek_token ();
  while (t->get_id () == Coolgate::LEFT_PAREN || t->get_id () == Coolgate::LEFT_SQUARE)
    {
      lexer.skip_token ();

      Tree lower_bound, upper_bound;
      if (t->get_id () == Coolgate::LEFT_SQUARE)
	{
	  Tree size = parse_integer_expression ();
	  skip_token (Coolgate::RIGHT_SQUARE);
...
Parser::query_type (const std::string &name, location_t loc)
{
  ...
  else if (sym->get_kind () != Coolgate::TYPENAME)
    {
...
Parser::query_variable (const std::string &name, location_t loc)
{
  ...
  else if (sym->get_kind () != Coolgate::VARIABLE)
    {
...
Parser::parse_assignment_statement ()
{
  ...
  const_TokenPtr assig_tok = expect_token (Coolgate::ASSIG);
  if (assig_tok == NULL)
    {
      skip_after_semicolon ();
      return Tree::error ();
...
Parser::parse_if_statement ()
{
  ...
  if (tok->get_id () == Coolgate::ELSE)
    {
      // Consume 'else'
      skip_token (Coolgate::ELSE);
  ...
  else if (tok->get_id () == Coolgate::END)
    {
      // Consume 'end'
      skip_token (Coolgate::END);
    }
...
Parser::left_binding_power (const_TokenPtr token)
{
  switch (token->get_id ())
    ...
    case Coolgate::DOT:
      return LBP_DOT;
    //
    case Coolgate::LEFT_SQUARE:
      return LBP_ARRAY_REF;
    //
    case Coolgate::ASTERISK:
      return LBP_MUL;
    case Coolgate::SLASH:
      return LBP_DIV;
    case Coolgate::PERCENT:
      return LBP_MOD;
    //
    case Coolgate::PLUS:
      return LBP_PLUS;
    ...
    case Coolgate::EQUAL:
      return LBP_EQUAL;
    case Coolgate::DIFFERENT:
      return LBP_DIFFERENT;
    case Coolgate::GREATER:
      return LBP_GREATER_THAN;
    case Coolgate::GREATER_OR_EQUAL:
      return LBP_GREATER_EQUAL;
    case Coolgate::LOWER:
      return LBP_LOWER_THAN;
    case Coolgate::LOWER_OR_EQUAL:
...
Parser::null_denotation (const_TokenPtr tok)
{
  switch (tok->get_id ())
	...
	return Tree (build_string_literal (::strlen (c_str) + 1, c_str),
      ...
      break;
    case Coolgate::TRUE_LITERAL :
      {
	return Tree (build_int_cst_type (boolean_type_node, 1),
		     tok->get_locus ());
      }
      break;
    case Coolgate::FALSE_LITERAL :
      {
	return Tree (build_int_cst_type (boolean_type_node, 0),
		     tok->get_locus ());
      }
      break;
    case Coolgate::LEFT_PAREN:
      {
	Tree expr = parse_expression ();
	tok = lexer.peek_token ();
	if (tok->get_id () != Coolgate::RIGHT_PAREN)
...
Parser::binary_array_ref (const const_TokenPtr tok, Tree left)
{
  ...
  if (!skip_token (Coolgate::RIGHT_SQUARE))
    return Tree::error ();
...
static void coolgate_parse_file (const char *filename);

void
coolgate_parse_files (int num_files, const char **files)
{
  for (int i = 0; i < num_files; i++)
    {
      coolgate_parse_file (files[i]);
    }
}

static void
coolgate_parse_file (const char *filename)
{
  // FIXME: handle stdin "-"
  ...
  Coolgate::Lexer lexer (filename, file);
  Coolgate::Parser parser (lexer);
